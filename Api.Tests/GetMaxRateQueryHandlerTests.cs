...
public class GetMaxRateQueryHandlerTests
{
    [Fact]
    public async Task ValidRequest_ShouldReturnAvgRate()
    {
        ...
        var handler = new GetMaxRateQueryHandler(mockMediator.Object);
        
        //Act

        var result = await handler.Handle(new GetMaxRateQuery(dto), CancellationToken.None);
        ...
        result.Data.Rate.Should().Be(fakeExchangeRate.Rates[2]);
    }
}