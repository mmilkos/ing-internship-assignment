...
public class GetMinRateQueryHandlerTests
{
    [Fact]
    public async Task ValidRequest_ShouldReturnAvgRate()
    {
        ...
        var handler = new GetMinRateQueryHandler(mockMediator.Object);
        
        //Act

        var result = await handler.Handle(new GetMinRateQuery(dto), CancellationToken.None);
        ...
        result.Data.Rate.Should().Be(fakeExchangeRate.Rates[0]);
    }
}